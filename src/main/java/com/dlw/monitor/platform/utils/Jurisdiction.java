...
public class Jurisdiction {
	...
	public static boolean hasJurisdiction(String menuUrl){
		...
		Boolean b = true;
		List<HashMap<String,Object>> menuList = (List<HashMap<String,Object>>)session.getAttribute(Const.SESSION_allmenuList); //获取菜单列表
		
		for(int i=0;i<menuList.size();i++){
			for(int j=0;j<((List<HashMap<String,String>>)menuList.get(i).get("SubMenu")).size();j++){
				if(((List<HashMap<String,String>>)menuList.get(i).get("SubMenu")).get(j).get("MENU_URL").split(".do")[0].equals(menuUrl.split(".do")[0])){
					if(!((List<HashMap<String,String>>)menuList.get(i).get("SubMenu")).get(j).get("HasMenu").equals("true")){				//判断有无此菜单权限
						return false;
					}else{																//按钮判断
						...
						map.remove("edit");
						map.remove("cha");
						String MENU_ID =  String.valueOf(((List<HashMap<String,String>>)menuList.get(i).get("SubMenu")).get(j).get("MENU_ID"));
			...
			for(int j=0;j<((List<HashMap<String,String>>)menuList.get(i).get("SubMenu")).size();j++){
				if(((List<HashMap<String,String>>)menuList.get(i).get("SubMenu")).get(j).get("MENU_URL").split(".do")[0].equals(menuUrl.split(".do")[0])){
					if(!((List<HashMap<String,String>>)menuList.get(i).get("SubMenu")).get(j).get("HasMenu").equals("true")){				//判断有无此菜单权限
						...
						Map<String, String> map = (Map<String, String>)session.getAttribute(Const.SESSION_QX);//按钮权限
						String MENU_ID =  ((List<HashMap<String,String>>)menuList.get(i).get("SubMenu")).get(j).get("MENU_ID");
						String USERNAME = session.getAttribute(Const.SESSION_USERNAME).toString();	//获取当前登录者loginname
						Boolean isAdmin = "admin".equals(USERNAME);
						if("add".equals(type)){
