	...
	<script type="text/javascript">
		...
		function getrzzbList(){
			...
			$.ajax({
				type : "post",
				url : uls,
				data : {fn_code:fn_code,fn_datetimeQ:fn_datetimeQ,fn_datetimeJ:fn_datetimeJ,fn_status:"8"},
				dataType : "text",
				async : false,
				success : function(dataInfo) {
					...
					for(var i = 0; i < data.length ; i++){
						...
						strHtml += "<td><a onclick='getfk(\""+data[i].fc_id+"\",\""+data[i].fn_id+"\",\""+data[i].fc_facustomerid+"\")'>付款</a> | ";
						strHtml += "<a onclick='getinfo1(\""+data[i].fc_id+"\")'>查看详情</a></td>";
						strHtml += "</tr>";
					}
		...
		//付款
		function getfk(fcid,fnid,fc_facustomerid){
			var uls = "<%=basePath %>exportfactoring/getfkinfo?fcid="+fcid+"&fnid="+fnid+"&fcfacustomerid="+fc_facustomerid;
