...
<div id="page-content">
  ...
  <div class="row-fluid">
	...
	<div>
	<!-- 组都可以分类，暂时屏蔽 吴旭 2015-05-21 改为角色
	<div id="breadcrumbs">
		...
		<table>
			...
			<td>
			<c:if test="${QX.edit == 1 }">
			<a class="btn btn-mini btn-info" onclick="editRole('${pd.ROLE_ID }');">修改角色名称<i class="icon-arrow-right  icon-on-right"></i></a>
			</c:if>
				<c:choose>
					...
					<c:otherwise>
					<c:if test="${QX.edit == 1 }">
					<a class="btn btn-mini btn-purple" onclick="editRights('${pd.ROLE_ID }');"><i class="icon-pencil"></i>角色菜单权限</a>
					</c:if>
					</c:otherwise>
				</c:choose>
				<c:choose> 
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function addRole(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增组";
			 diag.URL = '<%=basePath%>manager/role/toAdd.do?parent_id=0';
			 diag.Width = 222;
			 diag.Height = 90;
		...
		function addRole2(pid){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增角色";
			 diag.URL = '<%=basePath%>manager/role/toAdd.do?parent_id='+pid;
			 diag.Width = 222;
			 diag.Height = 90;
			 diag.CancelEvent = function(){ //关闭事件
				if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function editRole(ROLE_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>manager/role/toEdit.do?ROLE_ID='+ROLE_ID;
			 diag.Width = 222;
			 diag.Height = 90;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function delRole(ROLE_ID,msg,ROLE_NAME){
			bootbox.confirm("确定要删除["+ROLE_NAME+"]吗?", function(result) {
				if(result) {
					var url = "<%=basePath%>manager/role/delete.do?ROLE_ID="+ROLE_ID+"&guid="+new Date().getTime();
					top.jzts();
		...
		function kf_qx1(id,kefu_id,msg){
			...
			if(qxhc1 == 'checked'){
				...
				var url = "<%=basePath%>manager/role/kfqx.do?kefu_id="+kefu_id+"&msg="+msg+"&value="+value+"&guid="+new Date().getTime();
				$.get(url,function(data){
					if(data=="success"){
						//document.location.reload();
					}
		...
		function c1(id,msg,value,kefu_id){
				...
				var url = "<%=basePath%>manager/role/gysqxc.do?kefu_id="+kefu_id+"&msg="+msg+"&value="+value+"&guid="+new Date().getTime();
				$.get(url,function(data){
					if(data=="success"){
						//document.location.reload();
					}
		...
		function editRights(ROLE_ID){
			 ...
			 diag.Drag = true;
			 diag.Title = "菜单权限";
			 diag.URL = '<%=basePath%>manager/role/auth.do?ROLE_ID='+ROLE_ID;
			 diag.Width = 280;
			 diag.Height = 370;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		function roleButton(ROLE_ID,msg){
			...
			if(msg == 'add_qx'){
			 ...
			 diag.Drag = true;
			 diag.Title = Title;
			 diag.URL = '<%=basePath%>manager/role/button.do?ROLE_ID='+ROLE_ID+'&msg='+msg;
			 diag.Width = 200;
			 diag.Height = 370;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
